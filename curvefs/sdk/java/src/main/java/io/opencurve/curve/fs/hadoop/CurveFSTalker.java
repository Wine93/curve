...
class CurveFSTalker extends CurveFSProto {
    ...
    @Override
    int listdirplus(Path path, CurveFsDirent[] dirents, int size) throws IOException {
        logger.log("listdirplus", path, size);
        CurveFSStat stat = new CurveFSStat();
        try {
            mount.lstat(tostr(path), stat);
        } catch (FileNotFoundException e) {
        ...
        String[] names = mount.listdir(tostr(path));
        if (null == names) {
            return 0;
        }
        ...
        for (String name : names) {
            ...
            CurveFsDirent dirent = new CurveFsDirent();
            dirent.name = name;
            mount.lstat(tostr(path), dirent.stat);
            dirents[i] = dirent;
            i++;
        }
        return i;
